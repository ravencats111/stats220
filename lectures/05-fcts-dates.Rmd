...
.pull-left[
<br>
```{r box-movies-med, eval = FALSE}
```
]
.pull-right[
```{r ref.label = "box-movies-med", echo = FALSE, fig.height = 4.5}
```
]
...
.right-column[
`r emo::ji("arrow_down")` {lubridate} is NOT part of the core {tidyverse}
```{r lubridate}
```
...
## Format date-times (also convert to characters)
