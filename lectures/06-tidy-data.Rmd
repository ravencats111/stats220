...
```{r movies, echo = FALSE, highlight.output = 6}
```
...
* which rep is easier to work with?
...
.footnote[credit: Illustrations from the Openscapes blog [Tidy Data for reproducibility, efficiency, and collaboration](https://www.openscapes.org/blog/2020/10/12/tidy-data/) by Julia Lowndes and Allison Horst]
...
* It's much more pleasant to work with tidy data, 
* spent less flighting with different tools, focus more on data analyis bc one set of consistent tools
...
.right-column[
* `nest()` creates a .brown[list-column] of tibbles
```{r tb-nest}
```
]
...
* holding others constant, change over years
